...
export async function GET(
  ...
  try {
    ...
    // id is a UUID prefix (8 hex chars) generated by /api/files/upload.
    // Validate strictly: empty id or weird chars must not match arbitrary
    // files via `startsWith` / `includes`.
    if (!id || !/^[a-f0-9]{1,16}$/i.test(id)) {
